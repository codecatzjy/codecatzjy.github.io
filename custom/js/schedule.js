...
function cardRefreshTimes() {
    ...
    if (e) {
        ...
        e.querySelector(".schedule-r0 .schedule-d1 .aside-span2").innerHTML = `��ʣ<a> ${(365 - asideDay).toFixed(0)} </a>��`;
        ...
        e.querySelector(".schedule-r1 .schedule-d1 .aside-span2").innerHTML = `��ʣ<a> ${(dates - date)} </a>��`;
        e.querySelector("#pBar_week").value = week === 0 ? 7 : week;
        e.querySelector("#p_span_week").innerHTML = ((week === 0 ? 7 : week) / 7 * 100).toFixed(1) + "%";
        e.querySelector(".schedule-r2 .schedule-d1 .aside-span2").innerHTML = `��ʣ<a> ${(7 - (week === 0 ? 7 : week))} </a>��`;
    }
}
...
function cardTimes() {
    ...
    if (e) {
        ...
        e.querySelector("#calendar-week").innerHTML = `��${weekNum}��&nbsp;${weekStr}`;
        e.querySelector("#calendar-date").innerHTML = date.toString().padStart(2, "0");
        e.querySelector("#calendar-solar").innerHTML = `${year}��${monthStr}&nbsp;��${asideDay.toFixed(0)}��`;
        e.querySelector("#calendar-lunar").innerHTML = `${ganzhiYear}${animalYear}��&nbsp;${lunarMon}${lunarDay}`;
